...
export default function InterviewsClient({
  ...
  const [interviews, setInterviews] =
    useState<IInterviewData[]>(initialInterviews);
  const [isAddOpen, setIsAddOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'upcoming' | 'past' | 'completed'>(
    'upcoming'
  );
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage your upcoming interviews, track interview progress, and stay
            prepared.
          </p>
        </div>
        <Button
          onClick={() => setIsAddOpen(true)}
          className="h-10 bg-primary hover:bg-primary/95 text-white font-semibold rounded-lg shadow-sm hover:shadow-[0_4px_14px_0_rgba(37,99,235,0.2)] transition-all"
        >
      ...
      {filteredInterviews.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
          ...
          <h3 className="text-lg font-bold text-slate-900 mb-2">
            No interviews {activeTab === 'upcoming' ? 'scheduled yet' : 'found'}
          </h3>
