      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-2">
            <Link
              href="/"
              className="items-center gap-2 mb-6 group inline-flex"
            >
            ...
            <p className="text-slate-600 mb-8 max-w-sm text-balance">
              The all-in-one workspace for the modern job seeker. Track
              applications, optimize your resume with AI, and land your dream
              role faster.
            </p>
            ...
            <ul className="space-y-4">
              {footerLinks.product.map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="text-slate-500 hover:text-primary transition-colors text-[15px]"
                  >
                    {link.name}
                  </Link>
                </li>
        ...
        <div className="pt-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-[15px] text-slate-500">
            &copy; {new Date().getFullYear()} CareerFlow AI. All rights
            reserved.
          </p>
          <div className="flex items-center gap-2 text-[15px] text-slate-500 font-medium">
            <span>Built for smart professionals.</span>
          </div>
        </div>
      </div>
    </footer>
